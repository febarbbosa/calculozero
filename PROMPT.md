...
4. **Publicação.** Ver a seção abaixo — está pela metade.

### Estado da publicação

O repositório é `felipesilvas2/calculozero`, público. Ele **ainda não tem o
projeto inteiro**: subiram 11 dos 88 arquivos, e a pasta `src/` inteira (61
arquivos) ficou de fora. O site não está no ar.

O que aconteceu: os arquivos foram enviados arrastando para a interface web do
GitHub, e ela perde pastas com subpastas e arquivos que começam com ponto
(`.nojekyll`, `.github/`). Pelo terminal isso não acontece — `git add .` pega
tudo:
...
git init
git add .
git status --short | wc -l      # tem que dar 88
git commit -m "Calculo Zero: plataforma completa"
git branch -M main
git remote add origin https://github.com/felipesilvas2/calculozero.git
git push -u origin main
```

Depois, **Settings → Pages → Source: `Deploy from a branch`**, branch `main`,
pasta `/ (root)`. Vai por branch, e não por GitHub Actions: o site é estático e
não precisa de build, e o modo Actions depende de permissão para criar o site,
que é onde o repositório anterior travou.

**Atalho, se a pressa for só ver o site no ar:** `dist/calculozero.html` é a
plataforma inteira num arquivo autocontido. Renomeado para `index.html` na raiz
do repositório, ele publica sozinho — sem `src/`, sem mais nada. O `src/` serve
para o código ficar legível e versionado, não para o site funcionar.

### Decisões que não devem ser desfeitas

1. **Chave de API nunca no front-end.** `CZ.ai.configurar()` recusa qualquer
   coisa com formato de chave e aceita só endereço. O provedor remoto só entra
   em produção atrás de `api/tutor.js`, que roda em servidor e guarda o segredo.
   Publicar a chave no JavaScript entrega ela a todo visitante.
2. **Sem atribuição de ferramenta nos commits.** O histórico é do autor. Nada
   de `Co-Authored-By` de assistente, nada de autor que não seja
   `Felipe Silva <felipedasilva651@icloud.com>`.
3. **Scripts clássicos, não módulos ES.** Módulo exige servidor; script
   clássico abre com dois cliques. Isso é requisito, não preferência.
4. **Zero dependência no front-end.** Nada de CDN, nada de `npm install` para
   rodar. O `@anthropic-ai/sdk` em `api/` é a única dependência do projeto e
   vive fora do navegador.
5. **Tópico sem ficha nunca tranca o seguinte.** Com 38 fichas para 355
   tópicos, travar por pré-requisito vazio transformaria o mapa em muro.
6. **Domínio é calculado por desempenho**, com peso por dificuldade, e crédito
   de leitura limitado a 40 — ler nunca leva alguém acima de "Familiar".
7. **O nome do produto é Cálculo Zero**; o repositório é `calculozero`, sem
   acento e sem hífen.
